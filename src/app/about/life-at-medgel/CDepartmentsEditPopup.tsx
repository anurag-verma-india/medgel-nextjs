...
const DepartmentsEditPopup = () => {
  ...
  const handleDeleteDepartment = (departmentId: string) => {
    setDepartmentState(prev => {
      ...
      if (departmentId.startsWith('temp_')) {
        const filteredToCreate = prev.toCreate.filter((_, index) => 
          prev.current.findIndex(dept => dept._id === departmentId) !== prev.current.length - prev.toCreate.length + index
        );
        return {
          ...prev,
  ...
  const handleDeleteJob = (jobId: string) => {
    ...
    if (jobId.startsWith('temp_job_')) {
      setJobState(prev => ({
        ...prev,
        toCreate: prev.toCreate.filter(job => 
          !(job.department_id === selectedDepartmentId && job.job.designation === "New Position")
        )
      }));
    } else {
    ...
    try {
      ...
      if (departmentState.toCreate.length > 0 || jobState.toCreate.length > 0) {
        ...
        if (jobState.toCreate.length > 0) {
          createPayload.jobs = jobState.toCreate.map(job => ({
            department_id: job.department_id.startsWith('temp_') 
              ? departmentState.current.find(d => d._id === job.department_id)?._id 
              : job.department_id,
            ...job.job
          }));
        }
      ...
      if (hasErrors) {
        message.error("Some changes could not be saved. Please check the console for details.");
  ...
  if (departmentState.current.length === 0) {
    return (
      <div className="relative">
        <button
          onClick={() => setIsMainModalVisible(true)}
        ...
        <Modal
          ...
          footer={[
            <Button key="close" onClick={() => setIsMainModalVisible(false)}>
              Close
            </Button>
          ]}
        >
          ...
          {departmentState.current.map((department, index) => (
            <div key={department._id} className="mb-6 rounded border border-gray-200 p-4">
              ...
              <div className="mt-3 text-sm text-gray-600">
                Jobs in this department: {department.jobs.length}
              </div>
            </div>
          ))}
        </Form>

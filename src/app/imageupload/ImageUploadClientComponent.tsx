...
export default function ImageUploadClientComponent(): JSX.Element {
  ...
  const [validationOrServerError, setValidationOrServerError] = useState<string | null>(null);
  ...
  const handleImageSelect = async (
    ...
    setValidationOrServerError(null);
    ...
    if (!file.type.startsWith("image/")) {
      setValidationOrServerError("Please select an image file");
      return;
    }
    ...
    try {
      ...
      setValidationOrServerError("Failed to process image. Please try another file.");
    }
  ...
  const handleFormSubmit = async (
    ...
    if (!selectedFile) {
      setValidationOrServerError("Please select a valid image first");
      return;
    }
    setIsLoading(true);
    ...
    try {
      ...
      if(res && res.success){
        ...
        // setValidationOrServerError(null);
        setValidationOrServerError("Image uploaded successfully");
        setImage({
          src: res.filePath, // Path returned from the server
          alt: selectedFile.name || "Uploaded image",
        });
      } else {
        setValidationOrServerError(res.error || "Failed to upload image. Please try again.");
        // console.log("Image upload failed");
        // setValidationOrServerError("Failed to upload image. Please try again.");
        setIsUploaded(false);
        setIsLoading(false);
        setSelectedFile(null);
        // setValidationOrServerError(null);
        ...
        <div className="mb-6">
          ...
          {isUploaded && (
            ...
            <p className="mb-4 text-center text-sm text-green-600">
              Selected Image successfully uploaded to server!
              <br/>
              Original Image has been downloaded to your device.
            </p>
            <button onClick={previous} className="bg-green-400 text-white p-2 mb-3 w-full rounded-lg">View Changes</button>
            </>
          )}

          {/* Display validation error */}
          {validationOrServerError && (

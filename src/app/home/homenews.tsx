  ...
  return (
    <div className="homelatest-medgel">
      <div className="homecontent">
        ...
        <h2 className="text-red-500 mx-auto">LATEST AT MEDGEL</h2>
        {
          isAdmin && 
        <button onClick={()=>setOpenEditModal(true)} className="bg-[#1D8892] rounded-lg p-3 m-3 ml-auto text-white">Add News</button>
        }
        </div>
              ...
              {newsList.length > 0 ? (
                newsList.map((news, index) => (
                  <div className="homenews-item" key={index}>
                    ...
                    <p className="text-left">{news.description.slice(0,300)}...</p>
                    <div className="flex content-start">
                      <a href={`/news?id=${news._id}`} className="homeread-more">
                        Read More ❯
                      </a>
                    </div>
                  </div>
